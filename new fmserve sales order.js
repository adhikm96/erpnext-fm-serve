...
frappe.ui.form.on("Sales Order", "refresh", function(frm, cdt, cdn) {
    ...
    for(var i=0; i < p.payment.length; i++){
     if (p.docstatus==1 && p.payment[i].invoice_reference)
                {
                        frappe.call({
                    ...
                    args: {
                            doctype: "Sales Invoice", 
                              fields: ["name","status"],  
                            filters: {
                                name:["=", p.payment[i].invoice_reference]
                            }
                        },
...
{
frappe.msgprint("First submit the Sales Order and then Launch the Invoice");
}
 });
